...
:return:%HELP()
{
   local __return_____HELP___Synopsis='Perform a function return'
   local __return_____HELP___Usage='[OPTIONS] [<return-value>]'

   :help: --set "$__return_____HELP___Synopsis" --usage "$__return_____HELP___Usage" <<EOF
OPTIONS:
...
:return:()
{
   local __return________Options
   __return________Options=$(getopt -o '' -l 'emit:,stacktrace' -n "${FUNCNAME[0]}" -- "$@") || return
   eval set -- "$__return________Options"

   local __return________Message=
   local __return________Stacktrace=

   while true ; do
      case "$1" in
      --emit)        __return________Message="$2"; shift 2;;
      --stacktrace)  __return________Stacktrace='--stacktrace'; shift;;
      --)            shift; break;;
      *)             break;;
      esac
   done
   ...
   if [[ -n $__return________Message ]]; then
      if (( N == 0 )); then
         :log: "$__return________Message"

      else
         :error: $__return________Stacktrace "$N" "$__return________Message"
      fi

   else
      if [[ $N -gt 0 && -n $__return________Stacktrace ]]; then
         :error:stacktrace
      fi
